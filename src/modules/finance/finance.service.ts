...
export class FinanceService {
  ...
  async reconcileTaxObligations(
    ...
    const pendingObligations =
      await this.prisma.extended.taxObligation.findMany({
        where: { status: ObligationStatus.PENDING }, // companyId é injetado automaticamente
        orderBy: { dueDate: 'asc' },
      });
    ...
    for (const obligation of pendingObligations) {
      ...
      const isLocked = await this.prisma.extended.balanceLock.findUnique({
    ...
    this.prisma.clearCompanyScope(); // Limpa o escopo após o processamento
    return {
  ...
  async lockFinancialPeriod(
    ...
    try {
      return await this.prisma.extended.balanceLock.create({
        data: {
          companyId,
          month,
          year,
  ...
  async getModuleAuditTrail(companyId: string, module: string) {
    this.prisma.setCompanyScope(companyId);
    return await this.prisma.extended.auditLog.findMany({
      where: { module },
  ...
  async getFinancialHealthSummary(companyId: string) {
    ...
    const [balance, pendingTax] = await Promise.all([
      this.prisma.extended.bankTransaction.aggregate({
        _sum: { amount: true },
      }),
      this.prisma.extended.taxObligation.aggregate({
