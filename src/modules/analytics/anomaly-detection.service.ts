...
export class AnomalyDetectionService {
  ...
  async processCompanyAnomalies(companyId: string): Promise<AnomalyResult[]> {
    try {
      ...
      if (!transactions || transactions.length === 0) {
        this.logger.warn(`Massa de dados insuficiente para a empresa ${companyId}.`);
        return [];
      }
      ...
      this.logger.error(`Falha crítica no AnomalyDetectionService para empresa ${companyId}`, error instanceof Error ? error.stack : error);
      throw error;
    }
  }
  ...
  private detectDuplicity(txs: any[]): AnomalyResult[] {
    ...
    for (let i = 0; i < txs.length; i++) {
      for (let j = i + 1; j < txs.length; j++) {
        ...
        const sameAmount = new Decimal(t1.amount).equals(new Decimal(t2.amount));
        const sameDesc = t1.description === t2.description;
        
        const diffTime = Math.abs(t1.occurredAt.getTime() - t2.occurredAt.getTime());
        const diffDays = diffTime / (1000 * 60 * 60 * 24);

        if (sameAmount && sameDesc && diffDays <= this.CONFIG.DUPLICITY_WINDOW_DAYS) {
          results.push({
            type: 'DUPLICITY',
            severity: 'MEDIUM',
  ...
  private detectZScoreOutliers(txs: any[]): AnomalyResult[] {
    ...
    const stdDev = Math.sqrt(amounts.map(x => Math.pow(x - mean, 2)).reduce((a, b) => a + b, 0) / n);
    ...
    return txs
      ...
      .map(t => ({
        ...
        transactionId: t.id,
        metadata: { 
          zScore: stdDev === 0 ? 0 : ((Math.abs(Number(t.amount)) - mean) / stdDev).toFixed(2), 
  ...
  private detectBasicOutliers(txs: any[]): AnomalyResult[] {
    const sortedTxs = [...txs].sort((a, b) => Math.abs(Number(a.amount)) - Math.abs(Number(b.amount)));
    const median = Math.abs(Number(sortedTxs[Math.floor(sortedTxs.length / 2)].amount));
    
    return txs
      .filter(t => Math.abs(Number(t.amount)) > median * 5 && Math.abs(Number(t.amount)) > 500)
      .map(t => ({
        type: 'OUTLIER',
        severity: 'LOW',
  ...
  private async logAnomaliesToAudit(companyId: string, anomalies: AnomalyResult[]) {
    try {
      ...
      this.logger.error(`Erro ao salvar log de anomalias para ${companyId}`, err);
    }
  }
}
