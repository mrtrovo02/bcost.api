...
describe('EC2 operational check contract', () => {
  ...
  it('detects stale frontend deployments by comparing web health with web git head', () => {
    expect(scriptSource).toContain('function webGitShortSha');
    expect(scriptSource).toContain('BCOST_WEB_DIR');
    expect(scriptSource).toContain('$HOME/bcost.web/bcost-web');
    expect(scriptSource).toContain("expectedService === 'bcost-web'");
    expect(scriptSource).toContain('buildVersionMatches(body.buildVersion, expectedBuildVersion)');
  });

  it('allows compatible Git short SHA prefixes while still rejecting unrelated build versions', () => {
    expect(scriptSource).toContain('function buildVersionMatches');
    expect(scriptSource).toContain('actual.startsWith(expected) || expected.startsWith(actual)');
  });
});
