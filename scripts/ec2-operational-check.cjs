...
function currentGitShortSha() {
  const result = runShell('git rev-parse --short HEAD 2>/dev/null || true');
  const sha = result.stdout.trim();

  return result.status === 0 && sha ? sha : '';
}

function webGitShortSha() {
  const result = runShell(
    'cd "${BCOST_WEB_DIR:-$HOME/bcost.web/bcost-web}" 2>/dev/null && git rev-parse --short HEAD 2>/dev/null || true',
  );
  const sha = result.stdout.trim();
...
function buildVersionMatches(actual, expected) {
  if (!expected) return true;
  if (typeof actual !== 'string' || actual.length < 7 || expected.length < 7) return false;
...
async function checkHttp(results, name, url, expectedService) {
  try {
    ...
    const expectedBuildVersion =
      expectedService === 'bcost-api'
        ? currentGitShortSha()
        : expectedService === 'bcost-web'
          ? webGitShortSha()
          : '';
    const ok =
      response.ok &&
    ...
    record(
      ...
      `HTTP ${response.status}, status=${String(body.status)}, service=${String(body.service)}, build=${String(body.buildVersion)}${
        expectedBuildVersion ? `, expected=${expectedBuildVersion}` : ''
      }`,
    );
  } catch (error) {
