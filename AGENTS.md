...
- CI deve chamar `npm run test:release-gates`, nunca uma allowlist manual de specs de `src/release`, para que novos gates de producao entrem automaticamente na esteira.
...
- `npm run ops:ec2-check` compara `buildVersion` da API com o HEAD do backend e, quando `BCOST_WEB_DIR` ou `~/bcost.web/bcost-web` existir, compara tambem o `buildVersion` do frontend com o HEAD do web.
...
npx prisma generate
RELEASE_STAGE=beta npm run release:check
